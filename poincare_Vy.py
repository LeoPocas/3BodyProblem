...
# Detectar cruzamentos onde v_y = 0
crossings_vy = []

for i in range(1, len(t_eval)):
    if (v1[1, i-1] * v1[1, i]) < 0:  # Detectar cruzamento
        # Interpolação linear para encontrar cruzamento
        x_cross = r1[0, i-1] + (r1[0, i] - r1[0, i-1]) * (-v1[1, i-1]) / (v1[1, i] - v1[1, i-1])
        y_cross = r1[1, i-1] + (r1[1, i] - r1[1, i-1]) * (-v1[1, i-1]) / (v1[1, i] - v1[1, i-1])
        crossings_vy.append((x_cross, y_cross))

# Separar coordenadas dos cruzamentos
x_cross_vy, y_cross_vy = zip(*crossings_vy)

# Plotar seção de Poincaré
plt.figure(figsize=(10, 10))
scatter = plt.scatter(x_cross_vy, y_cross_vy, c=range(len(x_cross_vy)), cmap='viridis', s=10, alpha=0.9)
plt.colorbar(scatter, label='Ordem dos cruzamentos')
plt.title("Seção de Poincaré (v_y = 0, Corpo 1)")
plt.xlabel("x")
plt.ylabel("y")
plt.grid(True)

plt.savefig("poincare_section_vy.png", dpi=300, bbox_inches='tight')
plt.show()
